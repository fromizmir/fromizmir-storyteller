...
if st.button("Hikaye Üret"):
    ...
    # Seslendirme
    engine = pyttsx3.init()  # Bloğun dışında tanımla
    voices = engine.getProperty('voices')
    if not voices:
        st.error("Hiçbir ses bulunamadı, seslendirme devre dışı.")
    else:
        engine.setProperty("voice", voices[0].id)  # İlk mevcut sesi kullan
        with st.spinner("Seslendirme hazırlanıyor..."):
            audio_file = "story.mp3"
            engine.save_to_file(hikaye, audio_file)  # hikaye burada kullanılabilir
            engine.runAndWait()
        if os.path.exists(audio_file):
            st.audio(audio_file)
        else:
            st.error("Ses dosyası oluşturulamadı, hata detayları: kontrol edin.")
